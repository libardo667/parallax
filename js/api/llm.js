...
import { CALL_LOGS, CURRENT_RUN_ID, PROMPT_TEMPLATE_OVERRIDES, SOURCE_MATERIAL, activeSetupMode } from '../core/state.js';
...
export function validateApiConfig(cfg){if(!cfg.researchModel) return "Choose a research model before running the analysis.";if(!cfg.embeddingModel) return "Choose an embedding model before running the analysis.";if(normalizeMode(cfg)==="direct"&&!cfg.apiKey) return "Paste an OpenRouter API key in the setup panel for direct mode.";if(normalizeMode(cfg)==="direct"&&cfg.apiKey&&!cfg.apiKey.startsWith("sk-or-")) return "Direct mode expects an OpenRouter key that starts with sk-or-.";return "";}
