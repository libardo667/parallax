import { ACTIVE_ARTIFACT_KEY, ARTIFACT_STORE, CITATIONS, DISCS, LAST_RUN, RUN_STATE, TERMS, lastClaimsText, lastCritiqueText, lastOutlineText, lastReportText, setLastClaimsText, setLastCritiqueText, setLastMarkdownText, setLastOutlineText, setLastReportText, setLastRun } from '../core/state.js';
...
import { buildProbeSystemPrompt, buildProbeUserPrompt, buildRedTeamPrompt } from '../prompt/prompt-builders.js';
...
export async function runReplication(){
  ...
  if(!RUN_STATE?.probeResults?.length){showToast("Run a baseline analysis first.");return;}
  ...
  try{
    ...
    setLastRun({...(LAST_RUN||{}),replication:Array.isArray(out?.data)?out.data:[]});
  }catch(err){
...
export function artifactFilenameBase(key){const target=(LAST_RUN?.target||RUN_STATE?.target||"run").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"")||"run";return `parallax-${target}-${key}`;}
...
export async function generateReplicationArtifact(target,cfg){
  ...
  if(!RUN_STATE?.probeResults?.length) throw new Error("Run a baseline analysis first.");
    ...
    for(let runNo=1;runNo<=runCount;runNo++){
      try{
        ...
        const probeResults=await Promise.all(activeDiscs.map(async(d)=>{
          ...
          const norm=await getProbeResultWithRecovery({target,discName:d.name,probeSystem:probeBundle.systemPrompt||probeSystemBundle.systemPrompt||probeSystemDefault,userMsg:probeBundle.userPrompt,cfg:cfg2});
