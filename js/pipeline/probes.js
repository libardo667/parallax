...
function buildProbeRecoveryError(discName, parseAttempts = [], cfg = {}) {
  const err = new Error(`Probe ${discName} failed before any structured output was recovered.`);
  err.mode = cfg?.mode === "proxy" ? "proxy" : "direct";
  err.userMessage = "This probe never returned usable model output. Check your model settings or API key, then rerun for full coverage.";
  err.details = parseAttempts;
  return err;
}

export async function getProbeResultWithRecovery({target,discName,probeSystem,userMsg,cfg}){const parseAttempts=[];const rawSamples=[];const parseTry=(label,text)=>{try{return {parsed:extractJSON(text),recoveryMode:label};}catch(err1){try{return {parsed:extractJSONLenient(text),recoveryMode:`${label}-lenient`};}catch(err2){parseAttempts.push(`${label}: ${err1.message||err1}; lenient: ${err2.message||err2}`);return null;}}};const raw1=await callLLMJSON(probeSystem,userMsg,cfg);rawSamples.push(raw1);const parsed1=parseTry("initial",raw1);if(parsed1) return {...normalizeProbeResult(parsed1.parsed),recoveryMode:parsed1.recoveryMode};try{const repaired=await repairJSONWithModel(raw1,`Probe ${discName} on ${target}`,cfg);rawSamples.push(repaired);const parsedRepair=parseTry("repair",repaired);if(parsedRepair) return {...normalizeProbeResult(parsedRepair.parsed),recoveryMode:parsedRepair.recoveryMode};}catch(err){parseAttempts.push(`repair call failed: ${err.message||err}`);}const retryCfg={...cfg,__tempOverride:Math.max(0,Number(getQualityProfile(cfg.qualityMode).temperature||0.15)-0.08)};const raw2=await callLLMJSON(probeSystem,userMsg,retryCfg);rawSamples.push(raw2);const parsed2=parseTry("retry",raw2);if(parsed2) return {...normalizeProbeResult(parsed2.parsed),recoveryMode:parsed2.recoveryMode};try{const repaired2=await repairJSONWithModel(raw2,`Probe retry ${discName} on ${target}`,cfg);rawSamples.push(repaired2);const parsedRepair2=parseTry("retry-repair",repaired2);if(parsedRepair2) return {...normalizeProbeResult(parsedRepair2.parsed),recoveryMode:parsedRepair2.recoveryMode};}catch(err){parseAttempts.push(`retry-repair call failed: ${err.message||err}`);}try{const fallbackPrompt=buildProbeCompactFallbackPrompt(target,discName);const fallbackRaw=await callLLMJSON("You are a research expert. Return strict valid JSON only.",fallbackPrompt,{...cfg,__tempOverride:0,maxTokens:Math.min(getQualityProfile(cfg.qualityMode).maxTokens,1200)});rawSamples.push(fallbackRaw);const parsedFallback=parseTry("compact-fallback",fallbackRaw);if(parsedFallback){const norm=normalizeProbeResult(parsedFallback.parsed);if(norm.summary&&norm.terms?.length) return {...norm,recoveryMode:parsedFallback.recoveryMode};}}catch(err){parseAttempts.push(`compact-fallback call failed: ${err.message||err}`);}if(!rawSamples.length) throw buildProbeRecoveryError(discName,parseAttempts,cfg);const rescue=buildProbeRescueResult({target,discName,rawTexts:rawSamples,parseAttempts});console.warn(`Probe ${discName} JSON parse failed; using fallback recovery.`,parseAttempts);return {...rescue,recoveryMode:"rescue"};}
