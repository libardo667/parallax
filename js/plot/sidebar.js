...
import { CITATIONS, DISCS, LAST_RUN, RUN_STATE, TERMS, activeSlices, activeTab, activeTypes, galleryActive, nodeColorMode, plotInited } from '../core/state.js';
...
export function initNodeFilterControls(){const ids=["node-search-input","node-type-filter","node-min-probe-filter","node-min-citation-filter","node-sort-select"];for(const id of ids){const el=document.getElementById(id);if(!el) continue;const evt=(id==="node-search-input"||id==="node-min-probe-filter"||id==="node-min-citation-filter")?"input":"change";el.addEventListener(evt,()=>{if(plotInited&&activeTab==="plot"){if(galleryActive){document.dispatchEvent(new CustomEvent("gallery-filters-changed"));}else{renderPlot();}}renderNodeFilterResults();});}const clearBtn=document.getElementById("node-filter-clear-btn");if(clearBtn){clearBtn.addEventListener("click",()=>{clearNodeFilters();if(plotInited&&activeTab==="plot"){if(galleryActive){document.dispatchEvent(new CustomEvent("gallery-filters-changed"));}else{renderPlot();}}renderNodeFilterResults();});}}
...
export function buildSidebar(){const dl=document.getElementById("disc-list");dl.innerHTML="";for(const d of DISCS){const btn=document.createElement("div");btn.className="disc-btn";btn.dataset.id=d.id;btn.innerHTML=`<div class="disc-btn-dot" style="background:${d.col}"></div><span class="disc-btn-abbr" style="color:${d.col}">${d.abbr}</span><span class="disc-btn-name">${d.name}</span><button class="small-btn" data-rerun="${d.id}" type="button">RERUN</button>`;btn.addEventListener("click",()=>{if(activeSlices.has(d.id)) activeSlices.delete(d.id);else activeSlices.add(d.id);btn.classList.toggle("off");if(galleryActive){document.dispatchEvent(new CustomEvent("gallery-filters-changed"));}else{renderPlot();}});btn.querySelector("[data-rerun]").addEventListener("click",e=>{e.stopPropagation();rerunProbe(d.id);});dl.appendChild(btn);}const tl=document.getElementById("type-list");tl.innerHTML="";for(const [tp,cfg] of Object.entries(TYPE_CFG)){const btn=document.createElement("div");btn.className="type-btn";btn.dataset.type=tp;btn.innerHTML=`<div style="width:10px;height:10px;background:${cfg.col};margin-top:2px;flex-shrink:0;border-radius:2px"></div><div><div style="font-size:13px;letter-spacing:.4px;color:${cfg.col}">${cfg.label}</div><div style="font-size:12px;color:var(--muted);margin-top:2px">${cfg.desc}</div></div>`;btn.addEventListener("click",()=>{if(activeTypes.has(tp)) activeTypes.delete(tp);else activeTypes.add(tp);btn.classList.toggle("off");if(galleryActive){document.dispatchEvent(new CustomEvent("gallery-filters-changed"));}else{renderPlot();}});tl.appendChild(btn);}renderGroundingLegend();}
